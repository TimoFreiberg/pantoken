<script lang="ts">
  ...
  // Dev-only test hook for the live (fake-daemon) e2e tier. The Rust fake driver's
  // run_script vocabulary is deliberately sent through the existing `{type:"mock"}`
  // dev message so there is one server-side control seam. Gated on ?dev, so it never
  // attaches on a normal load; unknown names fail loudly in the fake control.
  onMount(() => {
    if (!dev) return;
  ...
  const scripts = ["reply", "reset", "markdown", "search", "thinkingtools", "skill", "confirm", "input", "editor", "qna", "qnatall", "answercard", "answerleadup", "ambient", "compat", "bgrun", "bgwait", "queue", "deliverqueue", "discardqueue", "initializing", "editdiff", "editbounds", "editcountguard", "editemptyguards", "editpatch", "images", "error", "idle", "streamhold", "staleidle", "pendinghold", "slowabort", "slownewsession", "toolhold", "timeout", "yesno", "inject", "contextfull", "contextover", "longoutput", "longthinking", "selectmany", "planhandoff", "planhandofftimeout", "planhandofflegacy", "planhandoffequal", "planhandoffcollision", "planhandoffpending", "planfacet", "planview", "goalactive", "goalclear", "context", "permission", "failnewsession", "failsession", "goal", "unknown", "jobs", "cleantools", "manysessions", "latenotify", "cwd", "cwdroot", "promptmaplong", "promptmaphold", "promptmaptoolonly"];
    ...
    if (dev) {
      ...
      onMount(() => {
        (window as unknown as { __pantokenHosts?: unknown }).__pantokenHosts = {
          ...
          getProfileCaptures: provider.getProfileCaptures,
          resetProfileCaptures: provider.resetProfileCaptures,
...
<style>
  ...
  .devbar {
    ...
    /* Cap the test-only rail's share of the viewport so the transcript above it keeps
       ...
       tall wrapped rail can squeeze .transcript-wrap so far that the prompt-map rail
       overlay positions up under the sticky app header, unreachable. 45vh
