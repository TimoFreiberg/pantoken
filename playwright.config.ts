...
// The e2e backend runs on an OS-assigned FREE port (PANTOKEN_AUTO_PORT=1 in the webServer
...
// overridable via PANTOKEN_E2E_VITE_PORT so a second checkout can run e2e concurrently without
// fighting over 15173 (e.g. PANTOKEN_E2E_VITE_PORT=25173).
const VITE_PORT = Number(process.env.PANTOKEN_E2E_VITE_PORT) || 15173;
...
// user's running pantoken instance.
export default defineConfig({
  ...
  webServer: {
    command:
      `PANTOKEN_DRIVER=mock PANTOKEN_AUTO_PORT=1 ` +
