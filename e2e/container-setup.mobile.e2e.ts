...
test("SSH row inputs have correct relative widths on phone", async ({ page }) => {
  await page.getByTestId("sidebar-open").click();
  const switcher = page.getByTestId("host-switcher");
  await switcher.getByTestId("host-switcher-trigger").click();
  await switcher.getByTestId("host-switcher-setup-docker").click();
  ...
  if (sshBox && portBox) {
    // Port should be approximately 80px.
...
test("Full-width actions have bounded height on phone", async ({ page }) => {
  await page.getByTestId("sidebar-open").click();
  const switcher = page.getByTestId("host-switcher");
  await switcher.getByTestId("host-switcher-trigger").click();
  await switcher.getByTestId("host-switcher-setup-docker").click();

  // Test SSH button: between 44px (touch minimum) and 64px (intrinsic max).
  const testBtn = page.getByTestId("cs-test-ssh");
  const testBox = await testBtn.boundingBox();
  expect(testBox).not.toBeNull();
  if (testBox) {
    expect(Math.round(testBox.height)).toBeGreaterThanOrEqual(44);
    expect(Math.round(testBox.height)).toBeLessThanOrEqual(64);
  }
});

test("All setup stage actions have bounded height on phone", async ({ page }) => {
  await setPendingRisksForNextDocker(page, [EPHEMERAL_RISK]);
  ...
  // Use this container button.
  await page.getByTestId("cs-container-work-api-dev").click();
  const useBtn = page.getByTestId("cs-use-container");
  const useBox = await useBtn.boundingBox();
  expect(useBox).not.toBeNull();
  if (useBox) {
    expect(Math.round(useBox.height)).toBeGreaterThanOrEqual(44);
    expect(Math.round(useBox.height)).toBeLessThanOrEqual(64);
  }

  // Exact-name stage.
  await page.getByTestId("cs-exact-name-link").click();
  await expect(page.getByTestId("cs-exact-input")).toBeVisible();
  const saveBtn = page.getByTestId("cs-save-later");
  const saveBox = await saveBtn.boundingBox();
  expect(saveBox).not.toBeNull();
  if (saveBox) {
    expect(Math.round(saveBox.height)).toBeGreaterThanOrEqual(44);
    expect(Math.round(saveBox.height)).toBeLessThanOrEqual(64);
  }

  // Risk stage — go back and select container, then use container to reach risks.
  await page.getByTestId("cs-back-to-list").click();
  await page.getByTestId("cs-container-work-api-dev").click();
  await page.getByTestId("cs-use-container").click();
  await expect(page.getByTestId("cs-risks-panel")).toBeVisible({ timeout: 10000 });

  // Both risk buttons.
  const chooseBtn = page.getByTestId("cs-choose-path");
  const chooseBox = await chooseBtn.boundingBox();
  expect(chooseBox).not.toBeNull();
  if (chooseBox) {
    expect(Math.round(chooseBox.height)).toBeGreaterThanOrEqual(44);
    expect(Math.round(chooseBox.height)).toBeLessThanOrEqual(64);
  }

  const acceptBtn = page.getByTestId("cs-accept-risks");
  const acceptBox = await acceptBtn.boundingBox();
  expect(acceptBox).not.toBeNull();
  if (acceptBox) {
    expect(Math.round(acceptBox.height)).toBeGreaterThanOrEqual(44);
    expect(Math.round(acceptBox.height)).toBeLessThanOrEqual(64);
  }
});
...
test("No horizontal page scrolling on phone", async ({ page }) => {
  ...
  // rather than a content-overflow stress test. The minmax(0, 1fr) grid track
  // is the primary guarantee against horizontal overflow.
  await page.getByTestId("cs-ssh-input").fill("very-long-username@very-long-hostname.example.com");

  const scrollWidth = await page.evaluate(() => document.documentElement.scrollWidth);
  const clientWidth = await page.evaluate(() => document.documentElement.clientWidth);
  expect(scrollWidth).toBeLessThanOrEqual(clientWidth);

  // The sheet body owns vertical scrolling, so check its horizontal geometry too.
  const body = page.getByTestId("computer-setup-panel").locator(".body");
  const bodyScrollWidth = await body.evaluate((el) => el.scrollWidth);
  const bodyClientWidth = await body.evaluate((el) => el.clientWidth);
