...
// corpus-backed fake daemon (PANTOKEN_DRIVER=fake). These
...
export async function gotoFreshLive(page: Page): Promise<void> {
  ...
  await page.goto("/?dev");
  // "Message pantoken…" is the idle composer placeholder; it only renders once the
...
export async function driveLive(page: Page, script: string): Promise<void> {
  await page.evaluate((s) => {
    const w = window as unknown as { __pantokenMock?: (script: string) => void };
    if (!w.__pantokenMock)
      throw new Error(
        "__pantokenMock hook missing — is the page on /?dev and the live client built?",
      );
    w.__pantokenMock(s);
  }, script);
}
