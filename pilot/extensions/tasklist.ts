 ...
 */
/**
 * Tasklist Extension — In-session task tracking for the agent (pilot-owned port of
 * ~/dotfiles/agents/extensions/tasklist.ts).
 *
 * Provides tools for the agent to maintain a focused task list during a session, with
 * periodic reminders every N turns of non-use. State is stored in a file outside the
 * session tree so it survives compaction and /resume.
 *
 * PORTING NOTE (docs/PLAN-self-contained-extensions.md, Chunk 3): no roles.mjs / model
 * dependency (tasklist is pure state management — it doesn't call a model), so unlike
 * session-namer/answer it needs none of the D2 `background-model` plumbing. Registered
 * via `additionalExtensionPaths` like the other owned extensions; the pilot-side toggle
 * ([OPEN E]) + the D3 "Pilot" badge + the @pilot frontmatter apply for free once the
 * basename is in `PILOT_OWNED_EXTENSION_NAMES`.
 *
 * [OPEN B] — id is INTERNAL-ONLY. The operator never sees a task's `#id` in the
 * human-facing widget: `tasklist_done`/`tasklist_delete` already fuzzy-match (exact id /
 * prefix / description substring via `findTask`), so a description fragment resolves a
 * task without the id being visible. The widget item lines (`updateWidget`) + the reminder
 * block (`formatReminder`) + the `/tasks` command therefore push just the description
 * (`  ○ fix foo`), NOT `  ○ #v23gry: fix foo`. The id stays internal — it's still
 * generated as before and surfaced to the AGENT via `tasklist_add`'s result + the
 * `tasklist_list`/disambiguation tool results (those paths are unchanged). Only the
 * human-facing widget surfaces drop it.
 *
 ...
 *
 * [OPEN B] note: because the operator never sees the `#id` in the widget, the typical
 * input is a description fragment (path 3). The id / prefix paths still work — the agent
 * has the ids from `tasklist_add`'s result and may pass them — but a description fragment
 * alone resolves a task, which is what makes dropping the visible id safe.
 */
function findTask(
  input: string,
...
/**
 * Format the task list for the `tasklist_list` TOOL RESULT (agent-facing). [OPEN B]:
 * this is the tool-result path, so it KEEPS the `#id` — the agent needs the ids to pass
 * back to `tasklist_done`/`tasklist_delete`. Only the human-facing widget surfaces drop
 * the id (see `updateWidget`/`formatReminder`).
 */
...
/**
 * Push the task list to the host's "tasklist" widget as plain `string[]` lines (the only
 * channel pilot's bridge renders remotely). [OPEN B]: the human-facing item lines drop the
 * `#id:` prefix — just `  ○ description` — since the operator resolves tasks by
 * description fragment via `findTask`, never by reading the id off the widget. The header
 * line (`Open Tasks (N):`) is not an item line and is unchanged.
 */
...
/**
 * Full reminder block injected on the exact trigger turn (into the system prompt).
 * [OPEN B]: the item lines drop the `#id:` prefix — just `  • description`. The agent
 * already holds the ids from `tasklist_add`'s result; the reminder is a nudge, not a
 * lookup table, so repeating the ids here buys nothing. The header line is unchanged.
 */
function formatReminder(items: TaskItem[]): string {
  if (items.length === 0) return "";
  const lines = [
    `Open Tasks (${items.length} remaining) — use tasklist_done/delete when addressed:`,
    ...items.map((t) => `  • ${t.description}`),
  ];
  return lines.join("\n");
}
...
export default function tasklistExtension(pi: ExtensionAPI) {
  ...
  pi.registerCommand("tasks", {
    ...
    handler: async (_args, ctx) => {
      ...
      // [OPEN B]: human-facing notify surface — drop the `#id:` (the operator resolves
      // tasks by description, same as the widget). Matches `updateWidget`'s item shape.
