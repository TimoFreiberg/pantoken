...
/// Parsed contents of a pantoken.pid lock file.
...
/// Acquire the PID lock at `dataDir/pantoken.pid`, reclaiming a stale one and
...
pub fn acquire_pid_lock(
    ...
    let lock_path = data_dir.join("pantoken.pid");
...
fn libc_kill(_pid: i64, _sig: i32) -> i32 {
    ...
    // Treat all locks as reclaimable — not ideal, but pantoken is Unix-only in practice.
    -1
}
...
mod tests {
    ...
    fn acquire_pid_lock_reclaims_stale() {
        ...
        fs::write(dir.path().join("pantoken.pid"), r#"{"pid": 999999}"#).unwrap();
        let lock = acquire_pid_lock(dir.path(), "test-id", 12345).unwrap();
        assert_eq!(lock.pid, 12345);
    }
    ...
    fn acquire_pid_lock_reclaims_self() {
        ...
        fs::write(dir.path().join("pantoken.pid"), r#"{"pid": 12345}"#).unwrap();
        let lock = acquire_pid_lock(dir.path(), "test-id", 12345).unwrap();
        assert_eq!(lock.pid, 12345);
    }
