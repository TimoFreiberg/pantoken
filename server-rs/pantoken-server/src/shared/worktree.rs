//! Create/remove isolated jj/git worktrees for pantoken sessions.
//!
...
/// A pantoken-created worktree, recorded at creation so it can later be flagged in the
...
/// Remove a pantoken-created worktree.
...
mod tests {
    ...
    fn plan_worktree_removal_jj_forgets_workspace_and_caller_removes_dir() {
        ...
        assert_eq!(
            p.args,
            strings(&["-R", "/Users/x/repo", "workspace", "forget", "pantoken-abc",])
        );
        assert!(p.remove_dir);
    }
