//! Library facade for the pantoken server.
//!
...
//! as `pantoken_server::…` for both the binary and the integration tests, so the
//! tests don't have to re-implement or duplicate driver internals.
//!
//! `main.rs` re-imports these via `use pantoken_server::{…}` and adds only
