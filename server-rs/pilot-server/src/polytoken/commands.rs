...
//! ```
//!
//! polytoken's commands are all daemon builtins. pilot's `CommandInfo.source`
...
//!
//! TUI builtins like /model + /models are deliberately omitted because pilot
//! has native UI for those (ModelPicker drives POST /model). The rest (/compact,
//! /rewind, /facet, /permissions, /title, /clear, …) have no first-class pilot
//! affordance OR are useful as quick-access text shortcuts, so they surface in
//! the slash menu.

use pilot_protocol::session_driver::{CommandInfo, CommandSource};
...
/// Commands pilot has a native first-class UI for, so they don't need a
/// slash-menu duplicate (pilot's affordance is richer than a text command).
...
pub fn parse_slash_commands(stdout: &str) -> Vec<CommandInfo> {
    ...
    for cmd in commands {
        ...
        // Strip the leading slash — pilot's typeahead matches on the bare name
