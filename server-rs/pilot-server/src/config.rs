...
//! the deploy sets PILOT_TOKEN and runs behind `tailscale serve`.
//
...
pub fn load() -> Config {
    let port = env_parse("PILOT_PORT", 8787);
    ...
    let client_dist = std::env::var("PILOT_CLIENT_DIST")
        ...
        .map(PathBuf::from)
        .unwrap_or_else(|| {
            // Default: ../../client/dist relative to the crate root (server-rs/pilot-server)
            ...
            // server-rs/pilot-server, so ../../client/dist = the repo's client/dist.
            let manifest_dir = Path::new(env!("CARGO_MANIFEST_DIR"));
            manifest_dir.join("../../client/dist")
        });
    let warm_cap = env_parse("PILOT_WARM_CAP", 8);
    let idle_reap_ms = env_parse("PILOT_IDLE_REAP_MS", 10 * 60 * 1000);
    let live_refresh_ms = env_parse("PILOT_LIVE_REFRESH_MS", 1000);
    let delta_flush_ms = env_parse("PILOT_DELTA_FLUSH_MS", 50);
