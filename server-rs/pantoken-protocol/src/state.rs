...
pub fn fold_event(state: &mut SessionState, ev: &SessionDriverEvent) {
    ...
    match ev {
        ...
        | E::RunCompleted { snapshot, .. } => {
            ...
            // The fold closes the open assistant on any non-running snapshot
            // but only interrupts running tools on runCompleted/runFailed/
            // sessionClosed. Orphaned tools from replay are settled by the
            // seed builder (history_to_seed_events emits a synthetic
            // ToolFinished(interrupted)). An idle sessionUpdated can be a
            // transient mid-tool snapshot (the daemon's isStreaming briefly
            // reads false during a rename/model change/auto-title), so
            // interrupting on it would kill a genuinely running tool — the
            // turnActive robustness design ORs independent in-flight signals
            // precisely so a single glitch can't hide the stop affordance.
            if snapshot.status != SessionStatus::Running {
                close_open_assistant(&mut state.items, Some(ev.timestamp()));
            }
            // runCompleted settles running tools (authoritative turn end) and
            // stamps branch handles (entryIds).
...
mod tests {
    ...
    #[test]
    fn fold_tool_finished_interrupted() {
        // The seed builder (history_to_seed_events) emits synthetic
        // ToolFinished with interrupted: Some(true) for orphaned tool_use
        // blocks whose tool_result was lost (e.g. to a context_cleared). The
        // fold must map this to Interrupted (–), not Error (✕), preserving
        // the exact status the replay path expects. Parity with the TS test
        // "a toolFinished with interrupted:true sets status interrupted".
        let mut s = initial_session_state();
        fold_event(
            &mut s,
            &E::ToolStarted {
        ...
        match &s.items[0] {
            TranscriptItem::Tool(t) => {
                assert_eq!(t.status, ToolStatus::Interrupted);
                assert!(t.finished_at.is_some());
            }
            _ => panic!("expected tool item"),
        }
    ...
    #[test]
    fn idle_session_updated_does_not_interrupt_running_tool() {
        ...
        match &s.items[0] {
            TranscriptItem::Tool(t) => {
                assert_eq!(
                    t.status,
                    ToolStatus::Running,
                    "an idle sessionUpdated must not interrupt a live tool"
                );
                assert!(t.finished_at.is_none(), "a live tool has no finishedAt");
            }
            _ => panic!("expected tool item"),
        }
    }
