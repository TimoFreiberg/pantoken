...
The tag triggers ci.yml's `release-prepare-linux` job for the signed Linux
x86_64 headless artifact. The macOS `release-prepare` job depends on that job
completing, stages its archive at `target/release/headless`, and builds the
macOS headless artifact before the desktop Tauri/Cargo package. This ordering is
load-bearing: `desktop/build.rs` hashes the staged Linux archive into the SSH
provisioning manifest instead of embedding a zero placeholder. The macOS job
then refreshes desktop hashes without rebuilding either headless artifact; the
publish job merges the retained macOS metadata with the untouched Linux metadata.
The web + desktop gates run alongside preparation, and once those gates pass,
`release` publishes the prepared artifacts via `publish.ts`. Running apps pick
the release up within a minute. One-time setup: the minisign key as an Actions
secret —
