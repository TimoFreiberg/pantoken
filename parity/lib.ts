...
//    exported into every harness-spawned process (pantoken, tmux panes, daemons).
 ...
 *  whole harness is one `rm -rf` from gone. Override with $PANTOKEN_PARITY_ROOT. */
export function parityRoot(): string {
  const explicit = process.env.PANTOKEN_PARITY_ROOT?.trim();
  if (explicit) return explicit;
  const stateHome =
    process.env.XDG_STATE_HOME?.trim() || join(homedir(), ".local", "state");
  return join(stateHome, "pantoken-parity");
}
...
//
// $PANTOKEN_PARITY_MODEL selects a preset; $PANTOKEN_PARITY_FULL + $PANTOKEN_PARITY_MINI (both, same
// provider) override with an explicit pair; $PANTOKEN_PARITY_CONFIG_DIR overrides wholesale.
...
/** Resolve the active model pair: an explicit `PANTOKEN_PARITY_FULL`+`PANTOKEN_PARITY_MINI` pair
 *  (same provider) if both are set, else the `PANTOKEN_PARITY_MODEL` preset (default deepseek). */
export function modelSpec(): ModelSpec {
  const full = process.env.PANTOKEN_PARITY_FULL?.trim();
  const mini = process.env.PANTOKEN_PARITY_MINI?.trim();
  if (full && mini) {
  ...
  const name = process.env.PANTOKEN_PARITY_MODEL?.trim() || "deepseek";
  const preset = PRESETS[name as ParityModel];
  if (!preset) {
    throw new Error(
      `PANTOKEN_PARITY_MODEL="${name}" unknown — presets: ${Object.keys(PRESETS).join(", ")} ` +
        `(or set both PANTOKEN_PARITY_FULL + PANTOKEN_PARITY_MINI to an explicit same-provider pair)`,
    );
  }
  return preset;
}
...
export function renderConfig(
  ...
  permissionMatcher: "bypass_plus" | "standard" = "bypass_plus",
): string {
  return `# Generated by the parity harness (parity/lib.ts) — pins a cheap (full, mini) pair so
# automated GUI⇄TUI test runs don't burn the prod default. Swap via PANTOKEN_PARITY_MODEL
# (deepseek|umans) or PANTOKEN_PARITY_FULL+PANTOKEN_PARITY_MINI, or point $PANTOKEN_PARITY_CONFIG_DIR
...
providers:
  ${spec.provider}:
    ...
    auth:
      type: static_key
      key: \${${spec.keyEnv}}
# ${permissionMatcher === "standard" ? "standard: applies your allow/ask/deny rules — it only GATES a tool when a permissions rule file (ask/deny) is also present. The corpus capture writes a permissions.yaml with an `ask` shell rule alongside this; the matcher alone does NOT prompt." : "bypass_plus: unattended — allows every call except those a deny rule blocks; `ask` rules are IGNORED under this matcher, and the runtime permission monitor governs notifications, not tool gating (verified 2026-07-06)."}
default_permission_matcher: ${permissionMatcher}
`;
}
...
export interface Paths {
   ...
   *  Override with $PANTOKEN_PARITY_CONFIG_DIR to point at a hand-maintained config (e.g. the
  ...
  /** pantoken's PANTOKEN_DATA_DIR (push keys, archive index) — under the root for clean teardown. */
  pantokenData: string;
...
export function paths(root = parityRoot()): Paths {
  ...
  // Override: $PANTOKEN_PARITY_CONFIG_DIR points at a hand-maintained config dir (no generate).
  const override = process.env.PANTOKEN_PARITY_CONFIG_DIR?.trim() || null;
  ...
  return {
    ...
    // --global-config-dir; identical to what pantoken derives from its inherited XDG_CONFIG_HOME.
    globalConfigDir: join(xdgConfig, "polytoken"),
    pantokenData: join(root, "pantoken-data"),
    runDir: join(root, "run"),
    envFile: join(root, "run", "env.json"),
    tmuxSocket: `pantoken-parity-${stableHash(root)}`,
  };
}
 ...
 *
 * If a daemon is already LIVE for the session (e.g. pantoken has it warm, or the TUI is
