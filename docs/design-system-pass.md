...
- **Single _labeled_ toggles** (`.filter-toggle` in Sidebar, `aria-pressed`; the hide-thinking switch in Settings, `role="switch"`/`aria-checked`) — a 2-state pill; the differing ARIA between the two is exactly what a future primitive should unify. Not IconButton (has a label), not SegmentedControl (single, not a set). **→ candidate `Toggle` primitive.**
