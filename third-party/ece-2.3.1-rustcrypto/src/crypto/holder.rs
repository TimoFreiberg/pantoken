...
#[cfg(all(feature = "backend-rustcrypto", test))]
#[inline]
fn autoinit_crypto() {
    let _ = set_cryptographer(Box::leak(Box::new(
        super::rustcrypto::RustCryptoCryptographer,
    )));
}

#[cfg(all(
    not(feature = "backend-openssl"),
    not(all(feature = "backend-rustcrypto", test))
))]
#[inline]
fn autoinit_crypto() {}
