...
compile_error!("ece backend features are mutually exclusive");
...
#[cfg(any(feature = "backend-rustcrypto", not(feature = "backend-openssl")))]
pub use holder::{set_boxed_cryptographer, set_cryptographer};

#[cfg(feature = "backend-rustcrypto")]
pub use rustcrypto::RustCryptoCryptographer;

/// Install the RustCrypto backend in ece's process-wide cryptographer holder.
///
/// This must be called once before any encryption or decryption operation.
#[cfg(feature = "backend-rustcrypto")]
pub fn init_rustcrypto() -> std::result::Result<(), holder::SetCryptographerError> {
    holder::set_boxed_cryptographer(Box::new(RustCryptoCryptographer))
}
...
mod tests {
    ...
    // Exact RFC 8291 ciphertext/decryption vector for the selected backend.
    #[test]
    fn test_default_cryptographer() {
